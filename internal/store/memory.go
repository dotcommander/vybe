...
//
//nolint:revive // argument-limit: all memory params (key, value, type, scope, scope_id, expires, pinned, kind) are required and distinct
func SetMemory(db *sql.DB, key, value, valueType, scope, scopeID string, expiresAt *time.Time, pinned bool, kind string) error {
	...
	return RetryWithBackoff(context.Background(), func() error {
		_, err := db.ExecContext(context.Background(), `
			INSERT INTO memory (key, value, value_type, scope, scope_id, expires_at, pinned, kind, updated_at)
		...
		`, key, value, valueType, scope, scopeID, expiresAt, pinned, kind)
		if err != nil {
...
//
//nolint:revive // argument-limit: all memory params (key, value, type, scope, scope_id, expires, pinned, kind, sourceEventID, sourceTaskID) are required and distinct
func UpsertMemoryTx(tx *sql.Tx, agentName, key, value, valueType, scope, scopeID string, expiresAt *time.Time, pinned bool, kind string, sourceEventID *int64, sourceTaskID string) (int64, error) {
	if key == "" {
	...
	_, err := tx.ExecContext(context.Background(), `
		INSERT INTO memory (key, value, value_type, scope, scope_id, expires_at, pinned, kind, source_event_id, source_task_id, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
		ON CONFLICT(scope, scope_id, key) DO UPDATE SET
			value = excluded.value,
			value_type = excluded.value_type,
			...
			updated_at = CURRENT_TIMESTAMP
	`, key, value, valueType, scope, scopeID, expiresAt, pinned, kind, sourceEventID, sourceTaskID)
	if err != nil {
	...
	}{Key: key, ValueType: valueType, Scope: scope, ScopeID: scopeID, ExpiresAt: expiresAt, Pinned: pinned, Kind: kind}
...
func UpsertMemoryWithEventIdempotent(db *sql.DB, agentName, requestID, key, value, valueType, scope, scopeID string, expiresAt *time.Time, pinned bool, kind string, sourceTaskID string) (int64, error) {
	type idemResult struct {
		EventID int64 `json:"event_id"`
	}
	r, err := RunIdempotent(context.Background(), db, agentName, requestID, "memory.upsert", func(tx *sql.Tx) (idemResult, error) {
		eid, txErr := UpsertMemoryTx(tx, agentName, key, value, valueType, scope, scopeID, expiresAt, pinned, kind, nil, sourceTaskID)
		if txErr != nil {
...
func GetMemory(db *sql.DB, key, scope, scopeID string) (*models.Memory, error) {
	...
	err := RetryWithBackoff(context.Background(), func() error {
		...
		if err := db.QueryRowContext(context.Background(), `
			SELECT id, key, value, value_type, scope, scope_id, expires_at, updated_at, created_at, pinned, kind, source_event_id, source_task_id
			FROM memory
...
func ListMemory(db *sql.DB, scope, scopeID string) ([]*models.Memory, error) {
	...
	err := RetryWithBackoff(context.Background(), func() error {
		...
		for rows.Next() {
			var mem models.Memory
			var sourceTaskID sql.NullString
			if err := rows.Scan(&mem.ID, &mem.Key, &mem.Value, &mem.ValueType, &mem.Scope, &mem.ScopeID, &mem.ExpiresAt, &mem.UpdatedAt, &mem.CreatedAt, &mem.Pinned, &mem.Kind, &mem.SourceEventID, &sourceTaskID); err != nil {
