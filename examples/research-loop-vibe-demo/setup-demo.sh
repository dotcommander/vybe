...
vibe agent init --agent "$VIBE_AGENT" --request-id "$(rid init)" >/dev/null

PROJECT_ID="$(vibe project create \
  --agent "$VIBE_AGENT" \
...
vibe task add-dep --agent "$VIBE_AGENT" --request-id "$(rid dep)" --id "$T2" --depends-on "$T1" >/dev/null
vibe task add-dep --agent "$VIBE_AGENT" --request-id "$(rid dep)" --id "$T3" --depends-on "$T2" >/dev/null
vibe task add-dep --agent "$VIBE_AGENT" --request-id "$(rid dep)" --id "$T4" --depends-on "$T3" >/dev/null
