...
export VIBE_AGENT="${VIBE_AGENT:-genealogy-loop-demo}"

TASKS_JSON="$(vibe task list)"
EVENTS_JSON="$(vibe events list --all --limit 500)"
RESUME_JSON="$(vibe resume --agent "$VIBE_AGENT" --request-id "eval_resume_$(date +%s%N)")"
