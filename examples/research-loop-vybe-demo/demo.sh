...
create_task() {
  ...
  vybe task create \
    ...
    --title "$title" \
    --desc "$desc" \
    --priority "$priority" | jq -r '.data.task.id'
}

# The loop processes pending tasks in priority order (highest first; ties by
# creation order) — see store.DetermineFocusTask. Descending priorities seed
# the intended T1 -> T2 -> T3 -> T4 sequence without a dependency table the
# loop never consults. (vybe task add-dep was removed from the CLI.)
T1="$(create_task \
  'ACF-1001 Amelia Earhart disappearance records' \
  'Search for death/burial records and update facts note' 40)"

T2="$(create_task \
  'ACF-1002 Frederick Douglass parentage verification' \
  'Confirm parent evidence and capture citation-level notes' 30)"

T3="$(create_task \
  'ACF-1003 Harriet Tubman death window BLOCKED_DEMO' \
  'Demonstrate blocked path when evidence requires unavailable source' 20)"

T4="$(create_task \
  'ACF-1004 Sojourner Truth 1870 census extraction' \
  'Extract and log household details for 1870 census candidate' 10)"
